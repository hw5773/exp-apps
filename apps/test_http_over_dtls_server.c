...
int main(int argc, char *argv[])
{
  ...
  server = open_udp_listener(port, 0);
  if (server < 0)
    abort();

  memset(&addr, 0, sizeof(addr));
  bio = BIO_new_dgram(server, BIO_NOCLOSE);
  timeout.tv_sec = 5;
  timeout.tv_usec = 0;
  ...
  while (DTLSv1_listen(ssl, (BIO_ADDR *) &addr) <= 0);

  client = socket(AF_INET, SOCK_DGRAM, 0);
  connect(client, (struct sockaddr *)&addr, sizeof(struct sockaddr_in));
  BIO_set_fd(SSL_get_rbio(ssl), client, BIO_NOCLOSE);
  BIO_ctrl(SSL_get_rbio(ssl), BIO_CTRL_DGRAM_SET_CONNECTED, 0, &addr);
  ...
  imsg(EXP_DEBUG_SERVER, "TLS session is established with %s", SSL_get_cipher(ssl));
